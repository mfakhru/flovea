...
- `main.py` — FastAPI app, mounts routers from `src/routers/` (`auth`, `categories`, `expenses`, `imports`, `incomes`, `balance`) plus a `/health` check that round-trips D1.
...
- `routers/balance.py` — the Saldo Istri ledger. Computes a running balance across pay periods and never stores one: every response re-derives `saldo_awal + Σ (istri_amount − Istri's expenses + koreksi)` from scratch, so correcting an old expense fixes every later period automatically. Its "keluar" figure uses the same `COALESCE(reimbursed_by, user_id)` attribution and the same `A_` special-case exclusion as `_fetch_summary`, so the ledger and `/expenses/summary` can never disagree — change one and you must change the other. Pending (unpaid) reimbursements are reported alongside the balance but deliberately not subtracted from it; they land only when marked lunas.
...
Data model (`migrations/0001_init.sql` plus later ALTERs in `0004`/`0005`/`0009`): `users` (exactly 2 rows, seeded manually — no self-registration), `categories` (6 defaults, users can add more), `expenses` (owned by a user, dated, categorized, optionally flagged for reimbursement and tagged with a `pay_period` of form `YYYY-MM` for salary-cycle grouping independent of calendar month).

`0008`/`0009` add the Saldo inputs: `incomes` holds one row per pay period with **two** independent figures — `amount` (household income, behind the Home trend chart) and `istri_amount` (the slice handed to Istri, the ledger's "diterima"). Each has its own upsert endpoint and neither may clobber the other. `balance_adjustments` holds signed manual corrections so the ledger can be reconciled against real cash without inventing expenses — never model a correction as an expense row, it would corrupt every per-category/per-user total. `app_settings` is a key/value table currently holding only `istri_opening_balance`. Migration numbering has a gap (`0003` doesn't exist) — this is expected, not a missing file.
...
- `lib/balance.ts` — the Saldo ledger endpoints. Every mutation returns the whole recomputed recap rather than the row it touched, because one edit moves every closing balance after it.
...
Within the API, expense attribution is **not** duplicated: `routers/balance.py` imports `NOT_SPECIAL_CASE_CLAUSE`, `SPECIAL_CASE_PREFIX` and `get_istri_id` from `routers/expenses.py` rather than restating them, so the Saldo ledger and the Riwayat totals move together.
